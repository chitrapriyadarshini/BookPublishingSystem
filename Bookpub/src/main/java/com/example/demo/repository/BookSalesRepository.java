...
public class BookSalesRepository {
	...
	/*public List<BookSalesResult> findBookSales(){
		...
		String sql = "SELECT  b.isbn, b.title, b.price, SUM(quantity) quantity, SUM(total_price) total_sales FROM books b, orders o , order_items ot WHERE o.id = ot.order_id AND ot.book_id= b.id GROUP BY b.name order by quantity desc";
